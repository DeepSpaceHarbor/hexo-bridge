...
export default function UploadStatus({result}: { result: UploadResults }) {
    function getFailures() {
        if (result.failed > 0) {
            return <>
                <Callout title={`${result.failed} ${result.failed === 1 ? "file has " : "files have"} failed.`}
                         icon="cross"
                         intent="danger">
                    {
    ...
    return <>
        <div className={Classes.DIALOG_BODY}>
            <Callout title={`${result.uploaded} ${result.uploaded === 1 ? "file has " : "files have"} been uploaded.`}
                     icon="tick-circle"/>
            {getFailures()}
        </div>
    </>;
