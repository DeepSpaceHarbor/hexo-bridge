...
import {Button, Classes, Dialog, InputGroup, Intent, Menu, Popover, Position} from "@blueprintjs/core";
...
export default function CreateNewPost() {
        ...
        return <Menu>
            {
                allScaffolds.map((scaffold: string) => {
                    ...
                    }} text={scaffold} key={scaffold} active={scaffold === selectedScaffold}/>
                })
            }
        </Menu>
    }
    ...
    return <>
        <Button minimal icon="insert" text="New post" onClick={() => {
            setIsOpen(true)
        }}/>
        <Dialog isOpen={isOpen} onClose={() => setIsOpen(false)} canOutsideClickClose={false}
                icon="insert" title="Create new post">
            <div className={Classes.DIALOG_BODY}>
                <p>Enter a name for the new post:</p>
                <InputGroup value={postTitle}
                            leftIcon="document" placeholder="<Insert post title>"
                ...
                <Popover content={getScaffoldMenuItems()} position={Position.RIGHT_TOP}>
                    <Button outlined text={`Use scaffold: ${selectedScaffold}`}
                            icon="applications"
