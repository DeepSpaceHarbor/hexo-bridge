...
export default function UnpublishPost({savePost}: { savePost: () => void }) {
    ...
    return <>
        <Button icon="pause" text={"Unpublish"}
                onClick={() => {
                    setShowUnpublishAlert(true)
                }}/>
        <Alert isOpen={showUnpublishAlert} intent={Intent.PRIMARY}
               confirmButtonText={"Unpublish"} cancelButtonText={"Cancel"}
               onCancel={() => setShowUnpublishAlert(false)}
               onConfirm={() => onUnpublish()}
        >
            <p><b>Are you sure you want to unpublish this post?</b></p>
            <p>Drafts can be seen and edited through the admin panel, but only published posts are available on your
                website.</p>
        </Alert>
    </>
}