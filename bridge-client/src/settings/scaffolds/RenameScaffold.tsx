...
export default function RenameScaffold({selectedScaffold, allScaffolds}: { selectedScaffold: { name: string }, allScaffolds: [] }) {
    ...
    return <>
        <Button icon={"edit"} text={"Rename"} minimal onClick={() => setIsRenameModalOpen(true)}
                disabled={selectedScaffold.name === "post" || selectedScaffold.name === "page"}
        />
        <Dialog isOpen={isRenameModalOpen} title={"Rename scaffold"} canOutsideClickClose={false}
